// jest.spyOn()を利用してMath.random関数をモック化
describe('Math.random with spyOn', () => {
  let spy;

  afterEach(() => {
    spy.mockRestore(); // モック関数を元の関数へ戻す
    // jest.restoreAllMocks(); // 他にモック化している関数があれば、jest.restoreAllMocksですべてのSpyOnを利用してモック化した関数を元の関数へ戻すことができる
  });
  ...
  it('Math.random return under 1', () => {
    ...
    // expect(Math.random() < 1).toBe(true) // 上記のtoBeLessThanをtoBeで評価した場合の例はこちら
  });
});
